...
RATE LIMITING (slowapi, per-IP): applied to /token (5/min, brute-force),
/auth/register (3/hour, spam signups), /leads/capture (10/min, public +
triggers a real Claude call), /ads/generate-creative (10/min, real Claude
call), /api/webhook/incoming (10/min, public intake), /api/chat (20/min,
public widget). Most authenticated read endpoints are NOT limited — this is
selective, aimed at the routes that are either public or cost real API/LLM
spend per call.

EMAIL VERIFICATION: registration now requires clicking a verification link
before login is allowed (UserModel.is_verified). If SMTP_HOST isn't set, the
link is printed to the server logs instead of emailed — fully testable
without a real mail provider configured yet. New env vars: SMTP_HOST,
SMTP_PORT, SMTP_USERNAME, SMTP_PASSWORD, SMTP_FROM_EMAIL, FRONTEND_URL
(used to build the verification link).

AUTO-MIGRATION: added run_lightweight_migrations(), which runs on startup
right after create_all(). create_all() only creates brand-new tables — it
does NOT add new columns to tables that already exist (this is exactly what
broke stripe_subscription_id earlier and required dropping every table,
destroying test data). Now, adding a new column to a model (like
is_verified here) gets picked up automatically on next deploy instead of
requiring a manual DROP TABLE. Not a real migration tool (Alembic) — no
rename/drop/type-change support, just handles the common case.

Run with:
    pip install fastapi uvicorn pydantic pydantic-settings httpx requests sqlalchemy \\
                "python-jose[cryptography]" "passlib[bcrypt]" stripe email-validator slowapi
...
from sqlalchemy import create_engine, Column as SAColumn, Integer, String, ForeignKey, DateTime, Text, Boolean, Float, text
...
# Per-IP rate limiting — protects against a single abusive user/bot running up
# real Anthropic/Stripe/ad-platform bills through public or costly endpoints.
# Applied selectively below (login, registration, public intake, AI-costing
# routes), not globally — most authenticated read endpoints don't need it.
...
class DBSettings(BaseSettings):
    ...
    FRONTEND_URL: str = "https://ihatework-frontend.onrender.com"
    EMAIL_VERIFICATION_EXPIRE_HOURS: int = 24
    REQUIRE_EMAIL_VERIFICATION: bool = False  # flip to true (env var) when ready to actually enforce it
    # SMTP is optional — if unset, verification links are printed to the
    # server logs instead of emailed, so registration/testing still works
    # end-to-end before you've configured a real mail provider.
    SMTP_HOST: str = ""
    SMTP_PORT: int = 587
    SMTP_USERNAME: str = ""
    SMTP_PASSWORD: str = ""
...
class UserModel(DBBase):
    ...
    is_verified = SAColumn(Boolean, default=False)
...
def run_lightweight_migrations():
    ...
    columns to tables that already exist. That's exactly what broke
    stripe_subscription_id earlier (required dropping all tables, destroying
    test data). This adds any missing columns on already-existing tables
    instead, so future model changes don't require another destructive drop.
    Intentionally minimal — not a real migration tool (no rename/drop/type-
    change support), just handles the common 'I added a new column' case."""
        ...
        for table in DBBase.metadata.sorted_tables:
            ...
            for column in table.columns:
                if column.name in existing_columns:
                    continue
                col_type = column.type.compile(dialect=db_engine.dialect)
                default_clause = ""
                ...
                try:
                    if is_postgres:
                        conn.execute(text(f'ALTER TABLE "{table.name}" ADD COLUMN IF NOT EXISTS "{column.name}" {col_type}{default_clause}'))
                    else:
...
def create_email_verification_token(email: str) -> str:
    """Separate 'purpose' claim from access tokens so a verification link can
    never be replayed as a login token, and vice versa."""
    expire = datetime.datetime.utcnow() + datetime.timedelta(hours=db_settings.EMAIL_VERIFICATION_EXPIRE_HOURS)
    return jwt.encode(
        {"sub": email, "purpose": "email_verification", "exp": expire},
        db_settings.JWT_SECRET_KEY, algorithm=db_settings.ALGORITHM,
    )

def decode_email_verification_token(token: str) -> str:
    """Returns the verified email, or raises HTTPException if invalid/expired/
    wrong purpose."""
    try:
...
def send_verification_email(to_email: str, token: str):
    """Real SMTP send if configured; otherwise prints the link to the server
    logs so registration/testing still works before you've set up a mail
    provider. Runs as a background task — never blocks the register response."""
    verify_link = f"{db_settings.FRONTEND_URL}/verify-email?token={token}"
    if not db_settings.SMTP_HOST:
        print(f"[Email Verification — SMTP not configured] Link for {to_email}: {verify_link}")
        return
    try:
        msg = MIMEText(
            f"Welcome to Ihatework!\n\nVerify your email by visiting:\n{verify_link}\n\n"
            f"This link expires in {db_settings.EMAIL_VERIFICATION_EXPIRE_HOURS} hours."
        )
        msg["Subject"] = "Verify your Ihatework account"
        msg["From"] = db_settings.SMTP_FROM_EMAIL or db_settings.SMTP_USERNAME
        msg["To"] = to_email
        with smtplib.SMTP(db_settings.SMTP_HOST, db_settings.SMTP_PORT) as server:
            server.starttls()
            if db_settings.SMTP_USERNAME:
                server.login(db_settings.SMTP_USERNAME, db_settings.SMTP_PASSWORD)
            server.sendmail(msg["From"], [to_email], msg.as_string())
        print(f"[Email Verification] Sent to {to_email}")
    except Exception as e:
        # Never let a mail-server hiccup break registration — log the link as
        # a fallback so the user isn't permanently locked out because of it.
        print(f"[Email Verification Send Failed] {e}. Fallback link for {to_email}: {verify_link}")
...
# 7. CHAT PROXY (Claude)
# =====================================================================
async def _call_claude_raw(payload: "ProxyMessagePayload") -> Dict[str, Any]:
    """The actual Claude API call. Used both by the authenticated HTTP route
    below AND called directly as a plain Python function by
    generate_creative_copy/analyze_lead_with_claude/call_claude_structured —
    those internal callers never go through the ASGI request cycle, so they
    can't satisfy a Depends(get_current_user) parameter. Keeping the real
    logic here, auth-free, and putting auth ONLY on the public route wrapper
    below is what makes both cases work correctly."""
...
async def proxy_claude(request: Request, payload: ProxyMessagePayload, current_user: UserModel = Depends(get_current_user)):
    """Was previously wide open — no auth, no rate limit, meaning anyone on
    the internet could burn your Anthropic budget through this endpoint
    directly. Now requires login and is rate-limited per-IP."""
    return await _call_claude_raw(payload)
...
class ProposedCRMUpdate(BaseModel):
    client_name: str
    status: str
    reasoning: str

async def extract_crm_update_from_message(message: str) -> ProposedCRMUpdate:
    """Was previously hardcoded to always propose the same fake
    {"client_name": "Acme Corp", "status": "Contract Signed"} regardless of
    what the message actually said — a real webhook could send any message
    and get back the identical canned action. Now genuinely reads the
    message via Claude, same pattern as lead triage / ad copy elsewhere in
    this file."""
    prompt = (
        f"A CRM/pipeline automation system sent this message about a client interaction:\n\"{message}\"\n\n"
        f"Infer the client/company name mentioned (or \"Unknown\" if none is identifiable) and the most likely "
        f"status update implied (e.g. \"Contract Signed\", \"Meeting Scheduled\", \"Deal Lost\", \"Follow-up Needed\" — "
        f"use your judgment, this is a free-text field, not a fixed enum). Briefly explain your reasoning in one sentence.\n\n"
        f"Respond with ONLY a valid JSON object with exactly these keys: "
        f"\"client_name\" (string), \"status\" (string), \"reasoning\" (string)."
    )
    result = await call_claude_structured(prompt, ProposedCRMUpdate, max_tokens=300, thinking_budget=500)
    if result:
        return result
    # Honest fallback — labeled as such, not presented as a real extraction.
    return ProposedCRMUpdate(
        client_name="Unknown",
        status="Needs manual review",
        reasoning="AI extraction unavailable (no API key configured) — this is an unprocessed fallback, not a real reading of the message.",
    )
...
async def pipeline_webhook_trigger(request: Request, payload: IncomingWebhookTenant):
    ...
    proposed = await extract_crm_update_from_message(payload.message)
    action = {
        ...
        "arguments": {"client_name": proposed.client_name, "status": proposed.status, "reasoning": proposed.reasoning},
    }
    with state_lock:
        review_queue.append(action)
    return {"status": "queued", "action_id": action["action_id"]}
...
async def analyze_lead_with_claude(lead_name: str, message: str) -> AIAnalysisResult:
    ...
    try:
        result = await _call_claude_raw(ProxyMessagePayload(messages=[{"role": "user", "content": prompt}], max_tokens=700, thinking_budget=1000))
...
async def call_claude_structured(prompt: str, schema_cls, max_tokens: int = 1000, thinking_budget: Optional[int] = 1500):
    ...
    if not ANTHROPIC_API_KEY:
        return None
    try:
        result = await _call_claude_raw(ProxyMessagePayload(
...
def register_tenant(request: Request, user_data: UserCreate, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    new_user = UserModel(email=user_data.email, hashed_password=hashed, tenant_id=new_tenant.id, is_verified=False)
    db.add(new_user)
    db.commit()
    ...
    return {
        "message": f"Tenant {user_data.tenant_name} registered successfully. Check your email to verify your account before logging in.",
...
def verify_email(token: str, db: Session = Depends(get_db)):
    ...
    if user.is_verified:
        return {"status": "already_verified", "message": "This account is already verified."}
...
def resend_verification(request: Request, payload: ResendVerificationRequest, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    # Deliberately generic response either way — confirming/denying an email
    # exists on this platform is its own small information leak.
...
def login(request: Request, form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    if db_settings.REQUIRE_EMAIL_VERIFICATION and not user.is_verified:
        raise HTTPException(status_code=403, detail="Email not verified. Check your inbox, or use POST /auth/resend-verification.")
...
@limiter.limit("10/minute")
def capture_lead(request: Request, lead_in: LeadIntake, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
...
async def generate_creative_copy(product_prompt: str, platform: str) -> AdCopySchema:
    ...
    try:
        result = await _call_claude_raw(ProxyMessagePayload(messages=[{"role": "user", "content": prompt}], max_tokens=500, thinking_budget=1200))
        text_blocks = [b["text"] for b in result.get("content", []) if b.get("type") == "text"]
        raw = "\n".join(text_blocks).strip()
        if raw.startswith("```"):
            raw = raw.strip("`").lstrip("json").strip()
...
@limiter.limit("10/minute")
async def api_generate_creative(request: Request, payload: CreateCreativeRequest, current_user: UserModel = Depends(get_current_user), db: Session = Depends(get_db)):
...
@app.post("/api/chat")
@limiter.limit("20/minute")
def process_chatbot_message(request: Request, payload: ChatMessagePayload):
...
def system_status(current_user: UserModel = Depends(get_current_user)):
    """Reports exactly what's configured vs. running on fallback — no more
    guessing whether ANTHROPIC_API_KEY, SMTP, Stripe, or ad-platform
    credentials actually landed. Requires login (not public) since knowing
    which integrations are/aren't configured is minor recon value for an
    attacker."""
    return {
        "database": {
        ...
        "ai": {
            ...
            "note": "If false, ad copy/lead triage/advisory all silently use fallback text instead of real Claude calls.",
        },
        "billing": {
            "stripe_configured": bool(db_settings.STRIPE_SECRET_KEY),
            "stripe_webhook_configured": bool(db_settings.STRIPE_WEBHOOK_SECRET),
            "metered_prices_configured": all([
                db_settings.STRIPE_PRICE_TRIAGE, db_settings.STRIPE_PRICE_AD_REFRESH,
                db_settings.STRIPE_PRICE_CHAT, db_settings.STRIPE_PRICE_IMAGE_GEN,
            ]),
        },
        "email": {
            "smtp_configured": bool(db_settings.SMTP_HOST),
            "require_email_verification": db_settings.REQUIRE_EMAIL_VERIFICATION,
            "note": "If smtp_configured is false, verification links print to server logs instead of emailing.",
        },
        "ad_platforms": {
            "meta_configured": bool(db_settings.META_ACCESS_TOKEN),
            "google_configured": bool(db_settings.GOOGLE_DEVELOPER_TOKEN),
            "tiktok_configured": bool(db_settings.TIKTOK_ACCESS_TOKEN),
            "linkedin_configured": bool(db_settings.LINKEDIN_ACCESS_TOKEN),
            "note": "Any platform showing false will use a mocked deployment id instead of a real API call.",
        },
        "your_account": {
            "email": current_user.email,
            "tenant": current_user.tenant.name,
            "tenant_subscription_status": current_user.tenant.stripe_subscription_status,
        },
    }
