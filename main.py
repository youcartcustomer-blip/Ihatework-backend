...
from fastapi import BackgroundTasks, Depends, FastAPI, HTTPException, status, Query
    ...
    try:
        if not seed_db.query(TenantModel).filter(TenantModel.name == "Demo Co").first():
            demo_tenant = TenantModel(
                name="Demo Co",
                stripe_item_triage=db_settings.STRIPE_ITEM_TRIAGE or None,
                stripe_item_ad_refresh=db_settings.STRIPE_ITEM_AD_REFRESH or None,
                stripe_item_chat=db_settings.STRIPE_ITEM_CHAT or None,
                stripe_item_image_gen=db_settings.STRIPE_ITEM_IMAGE_GEN or None,
            )
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                ...
                json={"model": "claude-sonnet-4-6", "max_tokens": payload.max_tokens, "messages": payload.messages},
                timeout=60.0,
            )
...
# (unchanged — still in-memory/unauthenticated; see docstring)
# =====================================================================
@app.get("/api/next-pending")
async def get_next_pending_actions():
    with state_lock:
        return list(review_queue)

@app.post("/api/approve/{action_id}")
async def approve_and_dispatch(action_id: str, approval: ReviewApproval):
    with state_lock:
        global review_queue
        review_queue = [t for t in review_queue if t.get("action_id") != action_id]
    return {"status": "success", "executed_action": action_id}

@app.delete("/api/reject/{action_id}")
async def discard_action(action_id: str):
    with state_lock:
        global review_queue
        review_queue = [t for t in review_queue if t.get("action_id") != action_id]
...
async def pipeline_webhook_trigger(payload: IncomingWebhook):
    mock_action = {
...
class DBSettings(BaseSettings):
    ...
    # Per-metric Stripe subscription item IDs (fixes: the source doc referenced
    # settings.STRIPE_ITEM_* without ever declaring them on the Settings class)
    STRIPE_ITEM_TRIAGE: str = ""
    STRIPE_ITEM_AD_REFRESH: str = ""
    STRIPE_ITEM_CHAT: str = ""
    STRIPE_ITEM_IMAGE_GEN: str = ""
...
class TenantModel(DBBase):
    ...
    stripe_subscription_item_id = SAColumn(String, nullable=True)  # legacy/general-purpose, kept for compat
...
async def analyze_lead_with_claude(lead_name: str, message: str) -> AIAnalysisResult:
    ...
    try:
        result = await proxy_claude(ProxyMessagePayload(messages=[{"role": "user", "content": prompt}], max_tokens=600))
...
async def call_claude_structured(prompt: str, schema_cls, max_tokens: int = 1000):
    ...
    if not ANTHROPIC_API_KEY:
        return None
    try:
        result = await proxy_claude(ProxyMessagePayload(messages=[{"role": "user", "content": prompt}], max_tokens=max_tokens))
        text_blocks = [b["text"] for b in result.get("content", []) if b.get("type") == "text"]
        raw = "\n".join(text_blocks).strip()
        if raw.startswith("```"):
            raw = raw.strip("`").lstrip("json").strip()
...
def register_tenant(user_data: UserCreate, db: Session = Depends(get_db)):
    ...
    db.add(new_user)
    db.commit()
    return {"message": f"Tenant {user_data.tenant_name} registered successfully, Stripe meters mapped."}
...
async def generate_creative_copy(product_prompt: str, platform: str) -> AdCopySchema:
    ...
    prompt = (
        f"You are an elite marketing copywriter specializing in {platform} ads.\n"
        f"Draft a high-converting ad for this product/service: '{product_prompt}'.\n\n"
        f"Constraints:\n"
    ...
    try:
        result = await proxy_claude(ProxyMessagePayload(messages=[{"role": "user", "content": prompt}], max_tokens=400))
        text_blocks = [b["text"] for b in result.get("content", []) if b.get("type") == "text"]
        raw = "\n".join(text_blocks).strip()
        if raw.startswith("```"):
            raw = raw.strip("`").lstrip("json").strip()
...
def get_roi_analytics():
    """NOTE: still global across ALL tenants and unauthenticated — the source doc
    didn't cover this endpoint, so it wasn't brought in scope for this merge."""
    analytics_db = DBSessionLocal()
    try:
        all_logs = analytics_db.query(DeploymentLogModel).all()
        total_logs = len(all_logs)
        successful_syncs = sum(1 for log in all_logs if log.status == "SUCCESS")
        total_leads = analytics_db.query(LeadModel).count()
    finally:
        analytics_db.close()
